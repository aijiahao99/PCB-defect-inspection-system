...
class MixUp(BaseMixTransform):
    ...
    def __init__(self, dataset, pre_transform=None, p: float = 0.0) -> None:
        ...
        Examples:
            ...
            >>> dataset = YOLODataset("path/to/new_data.yaml")
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    if dataset.use_keypoints:
        ...
        if len(flip_idx) == 0 and (hyp.fliplr > 0.0 or hyp.flipud > 0.0):
            ...
            LOGGER.warning("No 'flip_idx' array defined in new_data.yaml, disabling 'fliplr' and 'flipud' augmentations.")
        elif flip_idx and (len(flip_idx) != kpt_shape[0]):
            raise ValueError(f"new_data.yaml flip_idx={flip_idx} length must be equal to kpt_shape[0]={kpt_shape[0]}")
