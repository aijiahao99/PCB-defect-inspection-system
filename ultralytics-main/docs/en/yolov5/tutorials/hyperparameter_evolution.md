...
```bash
python train.py --epochs 10 --data k.yaml --weights yolov5s.pt --cache
```
...
```bash
# Single-GPU
python train.py --epochs 10 --data k.yaml --weights yolov5s.pt --cache --evolve

# Multi-GPU with delay
for i in {0..7}; do
  sleep $((30 * i)) # 30-second delay (optional)
  echo "Starting GPU $i..."
  nohup python train.py --epochs 10 --data k.yaml --weights yolov5s.pt --cache --device $i --evolve > "evolve_gpu_$i.log" &
done
...
#   (
#     while true; do
#       python train.py --epochs 10 --data k.yaml --weights yolov5s.pt --cache --device $i --evolve > "evolve_gpu_$i.log"
#     done
#   ) &
# done
```
